...
**/
path.join(__dirname, 'runtime/query-engine-rhel-openssl-1.1.x');
...
class PrismaClient {
    ...
    constructor(optionsArg) {
        ...
        if (useDebug) {
            runtime_1.debugLib.enable('prisma-client');
        }
        ...
        this.engineConfig = {
            ...
            datasources,
            generator: { "name": "client", "provider": "prisma-client-js", "output": "/home/vraveneau/prisma-workshop/prisma/node_modules/@prisma/client", "binaryTargets": [], "config": {} },
            ...
            for (const log of options.log) {
                ...
                if (level) {
                    this.on(level, (event) => {
                        ...
                        console.error(runtime_1.chalk[colorMap[level]](`prisma:${level}`.padEnd(13)) + (event.message || event.query));
                    });
                }
            }
        }
    ...
    get users() {
        throw new Error('"prisma.users" has been renamed to "prisma.user"');
    }
}
...
function UserDelegate(dmmf, fetcher, errorFormat, measurePerformance) {
    const User = {};
    User.findOne = (args) => args && args.select ? new UserClient(dmmf, fetcher, 'query', 'findOneUser', 'users.findOne', args || {}, [], errorFormat, measurePerformance) : new UserClient(dmmf, fetcher, 'query', 'findOneUser', 'users.findOne', args || {}, [], errorFormat, measurePerformance);
    User.findMany = (args) => new UserClient(dmmf, fetcher, 'query', 'findManyUser', 'users.findMany', args || {}, [], errorFormat, measurePerformance);
    User.create = (args) => args && args.select ? new UserClient(dmmf, fetcher, 'mutation', 'createOneUser', 'users.create', args || {}, [], errorFormat, measurePerformance) : new UserClient(dmmf, fetcher, 'mutation', 'createOneUser', 'users.create', args || {}, [], errorFormat, measurePerformance);
    User.delete = (args) => args && args.select ? new UserClient(dmmf, fetcher, 'mutation', 'deleteOneUser', 'users.delete', args || {}, [], errorFormat, measurePerformance) : new UserClient(dmmf, fetcher, 'mutation', 'deleteOneUser', 'users.delete', args || {}, [], errorFormat, measurePerformance);
    User.update = (args) => args && args.select ? new UserClient(dmmf, fetcher, 'mutation', 'updateOneUser', 'users.update', args || {}, [], errorFormat, measurePerformance) : new UserClient(dmmf, fetcher, 'mutation', 'updateOneUser', 'users.update', args || {}, [], errorFormat, measurePerformance);
    ...
    User.upsert = (args) => args && args.select ? new UserClient(dmmf, fetcher, 'mutation', 'upsertOneUser', 'users.upsert', args || {}, [], errorFormat, measurePerformance) : new UserClient(dmmf, fetcher, 'mutation', 'upsertOneUser', 'users.upsert', args || {}, [], errorFormat, measurePerformance);
